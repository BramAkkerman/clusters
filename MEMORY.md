...
**Exception**: `dawarich`'s app manifests live in a *separate* repo, `https://git.meneerak.nl/flux/dawarich` (self-hosted Gitea, private, under the `flux` org — not personal `bram/`) — the first app deliberately split out as a test of migrating off GitHub. Pattern for anything else that should live there too: create it under the `flux` org (not a personal namespace) so it stays separate from personal repos as more apps migrate; a thin pointer stays in *this* repo (a `GitRepository` pointing at the Gitea repo, e.g. `clusters/emk/dawarich-source.yaml`, plus the `Kustomization`'s `sourceRef` pointing at that `GitRepository` instead of `flux-system`) — everything else (the actual namespace/HelmRelease/Cluster manifests) lives only in the Gitea repo, at its root, not nested under `apps/<name>/`. Flux auth to Gitea: HTTPS + a Gitea personal access token (`write:repository` scope, created via `POST /api/v1/users/<user>/tokens`), stored in a `dawarich-gitea-auth` secret in `flux-system` — same bring-your-own-secret pattern as everything else, just a different credential shape (`username`/`password` for basic auth, not app-specific keys). `bram` is already an owner/member of the `flux` org, so the token still works fine after repos move there.
...
| `dawarich-gitea-auth` | flux-system | ak | username, password (Gitea PAT) |
