...
- Zitadel's default bootstrap admin (`zitadel-admin`, org `ZITADEL`) currently has no way to log in — `DefaultInstance.Org.Human.Password` and `.Email.Address` were both left unset, so it got created with neither a known password nor a real email to receive an init code at. Next wipe+redeploy should set `DefaultInstance.Org.Human.Password` explicitly (via secret) so there's a guaranteed working admin login independent of whether SMTP works.
